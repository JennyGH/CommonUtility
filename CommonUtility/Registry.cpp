...
#define REGISTRY_ERROR_INVALID_HKEY  -5
	...
	else
	{
		...
		switch (code)
		{
		case REGISTRY_ERROR_INVALID_HKEY:  m_what = _T("注册表键句柄已关闭或已删除"); break;
		default:
			break;
		}
	}
...
{
	/*
	 * dw = REG_CREATED_NEW_KEY(0x00000001L) : 说明该键是新创建
	 * dw = REG_OPENED_EXISTING_KEY(0x00000002L) : 说明该键本来存在
	 */
...
StringArray RegistryKey::GetSubKeyNames() const
{
	...
	DWORD charLength = ARRAYSIZE(keyName);  //想要读取多少字节并返回实际读取到的字符长度
	LONG rv = ERROR_SUCCESS;
...
StringArray RegistryKey::GetValueNames() const
{
	...
	DWORD dwIndexs = 0; //需要返回子项的索引 
	TCHAR valueName[MAX_PATH] = { 0 }; //保存子键的名称 
	DWORD dwValueNameLength = ARRAYSIZE(valueName);  //想要读取多少字节并返回实际读取到的字符长度
	LONG rv = ERROR_SUCCESS;
