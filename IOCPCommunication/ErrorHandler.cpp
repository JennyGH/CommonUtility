...
bool GetWSAErrorMessage(DWORD errcode, char buffer[], DWORD sizeOfBuffer)
{
	...
	common::raii::scope_stdcall_function<HLOCAL, HLOCAL> scope_free(LocalFree, lpMsgBuf);
	if (FormatMessageA(
	...
	//sprintf_s(buffer, sizeOfBuffer, "Failed to format message for errcode: %d.", GetLastError());
	return false;
}
